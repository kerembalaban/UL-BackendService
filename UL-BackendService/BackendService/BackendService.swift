...
public struct BackendService{
    public static let sharedInstance: BackendService = BackendService()
    ...
    public func fetchUserList(completion: @escaping ([User],Error?) -> Void) {
    ...
    public func fetchPostBySelectedUser(id:Int, completion: @escaping ([Post],Error?) -> Void) {
        if (!ConnectionUtils.sharedInstance.isInternetAvailable()){
            completion([],NSError(domain: self.INTERNET_CONNECTION_MESSAGE, code: 1000, userInfo: nil))
        }
    ...
    public func fetchCommentsBySelectedPost(id:Int, completion: @escaping ([Comment],Error?) -> Void) {
        if (!ConnectionUtils.sharedInstance.isInternetAvailable()){
            completion([],NSError(domain: self.INTERNET_CONNECTION_MESSAGE, code: 1000, userInfo: nil))
        }
    ...
    public func fetchAlbumsBySelectedUser(id:Int, completion: @escaping ([Album], Error?) -> Void) {
        if (!ConnectionUtils.sharedInstance.isInternetAvailable()){
            completion([],NSError(domain: self.INTERNET_CONNECTION_MESSAGE, code: 1000, userInfo: nil))
        }
    ...
    public func fetchPhotosBySelectedAlbum(id:Int, completion: @escaping ([Photo],Error?) -> Void) {
        if (!ConnectionUtils.sharedInstance.isInternetAvailable()){
            completion([],NSError(domain: self.INTERNET_CONNECTION_MESSAGE, code: 1000, userInfo: nil))
        }
